...
# arrays
def insert_one_itemset(slots, top_kui_per_slot_rev, top_kui_ptrs, kui_idx, CAS, stype):
    ...
    for stype in range(0, slot_types):
        while CAS[stype] > 0:
            insert_one_itemset(
                ...
                top_kui_ptrs, kui_idx, CAS, stype)
...
def _RHDPRIP(data_dict, kui_R, kui_H, dranks, slot_sizes):
    ...
    pick_r_or_h.extend(['D']*int(globals.H_RATIO * 10))
    kui_to_pick_ptr = 0
...
def DPRIP(data_dict, kui_idx, dranks, slot_sizes, method='INDIVIDUAL'):
    if method == 'RH':
        ...
        return _DPRIP(data_dict, kui_idx, dranks, slot_sizes)
